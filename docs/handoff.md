...
1a. **`docs/analysis-inventory.md`** — 分析機能の棚卸し。今あるもの／手持ちデータで
   足せるもの／データが要るものの仕分けと、残っているバイアスの自己申告
1b. **`docs/system-map.md`** — どこで何が動き、いつ走り、どの順に依存しているかの地図。
   定期実行の時刻表（Mac側を含む）と、そこから読める実行順序の問題もここ
