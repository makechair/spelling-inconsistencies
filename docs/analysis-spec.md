...
無人実行の既定値は、**1回20銘柄まで、そのうち新規銘柄は10件まで**（2026-08-03に
10/3から引き上げ）。新規をCSV順に増やし、既存は `last_success_utc` が古い順に
巡回する。APIがHTTPエラーを返した時点で残りを止めるため、未知のユニークシンボル
上限を一度に踏み抜かない。

**引き上げの理由**: イベントは**その銘柄の日足が既にある場合しかmatchしない**。
3件/回では53銘柄が揃うのに18回（約3.5週間）かかり、その間イベントスタディは
最初に投入した数銘柄だけを見続ける。分析レポートがMicronばかりになる原因の
半分はこれ（もう半分はteitenの収集源がメモリ寄りであること）。10件/回なら
約6回で揃う。帯域は1銘柄あたり全期間2.34MB（Phase 0実測）なので、
53銘柄でも約124MB／1GB枠。**律速はcalls数でも帯域でもなく未知のシンボル上限**
であり、そこはHTTPエラーで自動停止する設計が受け止める。
...
> **上記の処理量・コスト前提は 2026-08-03 に無効化された。** teiten は Haiku 4.5 を
> やめてローカル Qwen3 13B へ移行済み。次項「teiten のローカルQwen移行」が現行の事実。

### teiten のローカルQwen移行（2026-08-03、現行）

| 項目 | 旧（〜2026-07-31） | 現行 |
|---|---|---|
| モデル | Claude Haiku 4.5（API従量課金） | **ローカル Qwen3 13B** |
| 実行場所 | AWS Lambda `teiten-collector` | **ローカル実行** |
| スケジュール | 6/12/18時 JST の3回 | **12/18時 JST の2回** |
| `MAX_LLM_ITEMS` | 4 | **10** |
| 1日あたり最大要約数 | 12 | **20** |
| 対象範囲 | 半導体メモリ・装置中心 | メモリに限らず**ハイパースケーラも対象** |

**LLM推論コストが実質ゼロになったため、「token量を増やさない」制約は消滅した。**
`MAX_LLM_ITEMS` を据え置く理由（旧決定事項の3番目）はもはや無く、母数は
ローカルマシンの実行時間だけが上限。上の依頼事項3は破棄でよい。

**代わりに新しい制約が入った。** Qwen3 13B は Haiku 4.5 より構造化出力の
スキーマ遵守が弱い。これは corpus 側の取り込みに直接効く（下記）。

#### ⚠ 取り込みが全断するリスク（最優先の要改善点）

`src/usstocks/corpus/news.py` の `normalize_page()` は**1ページでも不正値を
見つけると `CorpusError` を投げ、その日の同期全体を中断する**。設計意図は
「schema driftを黙って分析データへ混ぜない」ことで、Haiku 4.5 の高い
スキーマ遵守を前提にすれば妥当だった。Qwen3 13B ではこの前提が崩れる。

中断を引き起こす条件（`news.py:196-232`）:

| 検査 | 中断条件 |
|---|---|
| `Tickers` | `TICKER_PATTERN` に合わない値が1つでもある（例: `NVDA株`、`Micron`、`N/A`） |
| `EventType` | `EVENT_TYPES` の8値以外（表記ゆれ、和訳、幻覚値） |
| `Sentiment` | `SENTIMENTS` の3値以外 |
| `Confidence` | 数値化できない、または 0..1 の範囲外 |

**1件の不正ページが20件分の取り込みを落とす**ため、母数を4→10へ増やした今は
遭遇確率も上がっている。Phase 3 のイベント数が伸びない場合、まずここを疑う。

対処の方向は2つあり、**両方やるのが望ましい**:

1. **teiten側（入口で正す）** — Qwen出力をNotionへ書く前にバリデーションし、
   enum外の値は正規化（小文字化・別名マッピング）するか、その項目だけ空にする。
   tickerは `universe.csv` の50銘柄と照合し、一致しないものは落とす。
   **ローカル実行なので、弾いた記事の再実行コストはゼロ**。
2. **corpus側（このリポジトリ）** — 全断をやめ、不正ページは
   `news_rejected.parquet` 等へ隔離して残りを取り込む。何件・なぜ落ちたかを
   記録すれば「黙って混ぜない」という当初の意図は保てる。現状は
   「黙って混ぜない」が「全部止まる」になっており、可用性を過剰に犠牲にしている。
...
- 実際のカバレッジ（`ai_platform`/`eda_ip` の記事が上位4件に入るようになったか）は
  ...
  **2026-08-03時点では上位10件、ハイパースケーラも対象**（上記のQwen移行を参照）。
- ~~**【最優先】Qwen の出力ゆれによる取り込み全断への対処。**~~
  **両方完了（2026-08-03）。** teiten側は `notion_normalize.py` で書込前に
  正規化（`73a79fc`）、corpus側は規約外ページを
  `corpus/news_rejected/part.parquet` へ隔離して残りを取り込むよう変更。
  **懸念は実在した** — `--check` の初回実測で 418ページ中1ページ
  （ticker `'SK HYNIX'`、Qwen移行後・teiten修正前に書かれたもの）が
  417ページ全部の取り込みを止めていた。

### 実測から分かった Phase 3 の本当の制約（2026-08-03）
...
|---|---:|---|
| 規約外で拒否 | 1 | 隔離実装で解消 |
| **tickerが1つも無い** | **362 / 417** | イベントスタディに使えない |
| `event_type` が空 | 352 | 新フィールド追加（7/31）以前のページ |
| ユニバース外ticker | 26種・約35件 | 形式は正しいが価格系列と結合されない |

**取り込みを直しても、価格と突き合わせられるのは約55ページ**（tickerを持つ分）。
Phase 3 の matched が伸びない主因は日足corpusの段階投入ではなく**こちら**。

ユニバース外の内訳は `SKH=5`, `SSNLF=5`(Samsung), `KIOX=3`/`KIOXIA=1`,
`POSCO=2` など、**韓国・日本のメモリ銘柄**が中心。teiten の収集源が
メモリ寄りである以上これは構造的で、`SKH`（SK Hynix）は3節の判断により
意図的にユニバース外なので永久にマッチしない。

打ち手の候補（未着手、要判断）:

1. **tickerが付く率を上げる** — 362/417 が無tickerなのは、記事が業界動向
   （価格・需給・統計）中心で個別企業に紐づかないためと推測される。
   Qwenのプロンプトで「言及された上場企業を必ず拾う」よう促す
2. **ユニバースを広げる** — `SSNLF`(Samsung) 等を追加するかは、Tiingoが
   その銘柄を返すか次第（`SKH` と同じ問題を踏む可能性）
3. **無tickerイベントをセクター単位で扱う** — 個別銘柄ではなく
   subsector平均へのイベントとして集計する。母数を最大化できるが設計変更
...
銘柄別の全日足について、初日リターン中央90%を5〜95 percentile間の17点で評価する。
各評価点は外れ値に頑健なSilverman bandwidth（標準偏差とIQR/1.34の小さい方、下限0.30%）の
Gaussian kernelで近接日を重み付けし、
横軸を連続的な変動率、縦軸を1〜20取引日、色を同じhorizonの無条件平均に対する
超過forward returnとする。上下5%は急落・急騰の独立帯に分け、実効標本不足なら灰色にする。
これにより、-4%と-20%を同じ下位10分位に入れる問題と、境界直前・直後で結果が急変する問題を
避ける。長期の株価上昇ドリフトは色から除き、初日変動率に固有の平均回帰・継続を見せる。
各評価点・テール帯について、1〜19日後を買い日B、Bより後の2〜20日後を売り日Sとする全組み合わせを
日足から直接集計する。売買区間の平均・中央値・勝率・10%下振れ点・同subsector超過を求め、
往復コスト0.10%と期間重複を考慮した標準誤差を差し引いた80%片側下限が最大のB→Sを重ねる。
判断カードには同じ基準で上位3ペアを期待値・勝率・10%下振れ点・80%下限付きで並べ、
平均が良い次点候補と再現性を含めた最良候補の差を比較できるようにする。
これにより、独立した累積returnの最小・最大を選んでB=1日、S=20日に機械的に偏る問題を避ける。
実効標本が10件未満のペアは候補から除外する。Sが20日後なら天井確定ではなく
「観測期間末でも上昇中」と表示し、右端打ち切りを売却根拠にしない。

Webでは変動率帯をselectまたはヒートマップ列のクリックで選択できる。選択帯について
絶対期待returnと同期間の通常平均を1〜20日後の折れ線で重ね、B/Sを縦線で示す。
1〜5日、6〜13日、14〜20日の通常平均との差から、継続優位、継続劣位、短期調整後の回復、
初動優位後の失速、改善、悪化、方向不安定をルールベース分類する。隣接評価点との経路相関と
B/Sが3日以内に一致する帯数を「安定性」として表示する。これはLLM考察ではなく再現可能な
決定論的要約である。多数候補から最良ペアを選ぶ探索バイアスは残るため、ペア内信頼下限を
最終的な売買推奨とは呼ばない。

探索結果の精度は3区間のexpanding-window検証で別に測る。最初の55%を学習期間とし、
55〜70%、70〜85%、85〜100%を順に未使用の検証期間とする。各foldでは検証開始より
20取引日以上前の履歴だけから5/25/75/95 percentile、bandwidth、B→Sを再計算する。
急落、弱含み、中立、強含み、急騰の5条件について、往復コスト0.10%控除後の学習期待値と
検証実績平均、勝率、10%下振れ点、期待値誤差、方向一致を出す。検証区間の各シグナルは
個別例として変動日・初日変動率・B/S・実現returnを保持する。これはポジション重複、
資金制約、約定滑りを再現するportfolio backtestではなく、条件付き期待値のout-of-sample
校正であることをWebに明記する。
...
## 同業要因の分解をbetaで補正（2026-08-06）

`abnormal_return` は同subsectorの等ウェイト平均を**そのまま引いていた**。これは
「どの銘柄も同業と1対1で動く」という仮定で、実際には成り立たない。連動の大きい
銘柄は上昇日に同業より大きく上がるので、**セクターが上がるたびに個別要因プラス、
下がるたびに個別要因マイナス**と報告され続ける。これはニュースではなく仮定の
副産物である。

- 反応日**より前**の120営業日で、日次リターンを同業平均（自分を除く）に回帰して
  betaを推定する。イベント当日は推定に入らない
- 観測が60日未満、または0.2〜3.0の範囲外なら**1として扱い**、`beta_source` に
  `assumed` と記録する。60日のノイズから出た外れ値のbetaは、置き換えるはずの
  仮定より大きく数字を動かす
- `abnormal_return = raw_return − beta × 同業平均`
- `sector_attributed_return`（= beta × 同業平均）を併記する。分解は足して元に
  戻らなければ、1つの値の分割ではなく2つの別の値でしかない
- 旧定義は `equal_weight_abnormal_return` として残す。数字が変わったとき、
  betaのせいか当日のニュースのせいかを切り分けるため

**残る近似**: betaは日次リターンで推定し、複数日の累計リターンに適用する。
複利の二次項が残るが、補正しない場合の偏りより一桁小さい（テストで固定）。

同業平均そのものの汚染（セクター全体のニュースでpeerも動く）は、この補正では
解けない。それは分解の限界であって、推定の問題ではない。

## 銘柄別のニュース接続状況（2026-08-06）

「記事が少ない」と「記事はあるが日足に接続していない」は**別の問題**で、直す
場所も違う（前者はクロール側、後者はコーパス側）。合計値ではどちらも同じ不足に
見えるため、`symbol_news_coverage` として銘柄ごとに分けて数える。

| 列 | 意味 |
|---|---|
| `events` | その銘柄を指す記事から作られたイベント数 |
| `matched_events` | 反応取引日を決められた数 |
| `attach_rate` | `matched_events / events` |
| `no_price_series` | 日足が1本も無いため接続できなかった数 |
| `no_session_after_event` | 日足はあるが、候補日以降に立会日が無い数 |
| `sessions` | 日足の本数 |
| `diagnosis` | 直す順に1つだけ選ぶ |

`diagnosis` は上から順に `no_price_series` → `partly_unpriced` → `no_articles`
→ `some_events_dropped` → `short_price_history`(252営業日未満) → `few_articles`
(5件未満) → `ok`。**日足が無い銘柄は何も接続できない**ので、記事数より先に
報告する。

### ヒートマップ（初日変動率×保有日数）が埋まらない銘柄

条件付き集計は `effective ≥ 10` でしか描かない。この `effective` は
**局所実効標本を保有日数で割った値**（`local_effective_observations / horizon`）で、
窓の重なりを補正している。したがって**保有日数が長いセルほど多くの日足を要求する**。

上場・分離から日が浅い銘柄（SNDKなど）が埋まらないのは、ニュースの量ではなく
**日足の本数**の問題である。記事を増やしても埋まらない。`symbol_news_coverage` の
`sessions` と `short_price_history` がその判別に使える。
