...
無料枠 1GB/月 が30銘柄で成立するかは、仕様書では検証されていない。
...
| 実測（N銘柄・1日） | 30銘柄・20営業日の外挿 | 判断 |
|---|---|---|
| `bytes × 30/N × 20` < 0.8 GB | 枠内 | そのまま進める |
| 0.8〜1.0 GB | 枠ぎりぎり | 銘柄数を減らすか、警告閾値を下げて運用する |
| > 1.0 GB | 超過 | **意思決定が必要**: 銘柄数削減／有料枠／主系をAlpacaへ変更 |
