...
### 10-1. なぜYahoo!ファイナンスを使わないか

利用規約が自動収集を禁じており、リンク先PDFのS3複製は再配布の論点も伴う。
本プロジェクトは `analysis-spec.md` で
「paywall、認証回避、robots／利用条件に反する取得は行わない」と既に定めている。
加えてHTMLは構造変更で黙って0件になる経路であり、`SK HYNIX` 型の
静かな停止を新規に作ることになる。**XBRLの公式配信を使う。**
...
| J-Quants 有料プラン | 月額 | 当日〜翌日 | **費用が発生する。** 将来の日本株株価もここで賄える |
