...
- **A-1 帯域**: 無料枠の 1GB/月 は、30銘柄の常時購読では最も楽観的な仮定でも使い切る。Collector に実測メーターを入れ、`/api/health` で監視できるようにした。フェーズ1で必ず実測すること。
- **A-2 RESTレート**: 50回/時では、再接続のたびに30銘柄を補完すると即座に枯渇する。永続トークンバケットと要求の合体で対処した。
